...
export const GamesTable: React.FC<GamesTableProps> = ({ games, currentTeamId, loadingGames, reloadGames }): ReactElement => {
    return (
            ...
            <div className="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                <div className="py-2 align-middle inline-block min-w-full max-w-6xl px-1 sm:px-6 lg:px-8">
                    <div className="shadow sm:rounded-lg">
                        {games && games.length > 0 && !loadingGames && (
                            <table className="min-w-full divide-y divide-gray-600">
                                <thead className="bg-gray-50 dark:bg-black">
                                    <tr>
                                        ...
                                        <th
                                            scope="col"
                                            className="py-5 whitespace-nowrap text-center text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider font-header"
                                        >
                                            Result
                                        </th>
                                        <th
                                ...
                                <tbody className="bg-white dark:bg-black divide-y divide-gray-600">
                                    {games &&
                                        games.map((game) => (
                                            <GameRow reloadGames={reloadGames} key={game.id} currentTeamId={currentTeamId} game={game} />
                                        ))}
                                </tbody>
                            </table>
                        )}
                        <TableLoadingSpinner loading={loadingGames} />
                        {(!games || games.length === 0) && !loadingGames && <NoGamesFoundPlaceholder />}
                    </div>
                </div>
            </div>
        </div>
