...
import { PlayerProgression } from 'src/lib/api';
import { displayPlayerStat, getSortedPositions } from 'src/lib/utils';
import { ReadonlyPlayer } from 'src/services/PlayerService';
import { PlayerBackCardAttributeDelta } from '../common/PlayerBackCardAttributeDelta';
...
interface PlayerBackCardProps {
    ...
    delta: PlayerProgression;
}
export const PlayerBackCardDelta: React.FC<PlayerBackCardProps> = ({ player, playerImageURL, withGradient, delta }) => {
    ...
    const stats = [
        {
            ...
            deltaField: 'three_pt_delta',
        },
        {
            title: '2PT-INT',
            field: 'interior_2pt_rating',
            deltaField: 'interior_2pt_delta',
        },
        {
            title: '2PT-MID',
            field: 'midrange_2pt_rating',
            deltaField: 'midrange_2pt_delta',
        },
        {
            title: 'FT',
            field: 'ft_rating',
        ...
        },
        {
            title: 'DREB',
            field: 'drb_rating',
            deltaField: 'drb_delta',
        },
        {
            title: 'OREB',
            field: 'orb_rating',
            deltaField: 'orb_delta',
        },
        {
            title: 'PASS',
            field: 'ast_rating',
            deltaField: 'ast_delta',
        },
        {
            title: 'IDEF',
            field: 'interior_defense_rating',
            deltaField: 'interior_defense_delta',
        },
        {
            title: 'PDEF',
            field: 'perimeter_defense_rating',
            deltaField: 'perimeter_defense_delta',
        },
        {
            title: 'PHY',
            field: 'physicality_rating',
            deltaField: 'physicality_delta',
        },
        {
            title: 'LONG',
            field: 'longevity_rating',
            deltaField: 'longevity_delta',
        },
        {
            title: 'HSTL',
            field: 'hustle_rating',
            deltaField: 'hustle_delta',
        },
        {
            title: 'IQ',
            field: 'bball_iq_rating',
            deltaField: 'bball_iq_delta',
        },
        {
            title: 'LDRS',
            field: 'leadership_rating',
            deltaField: 'leadership_delta',
        },
        {
            title: 'COACH',
            field: 'coachability_rating',
            deltaField: 'coachability_delta',
        },
    ];
                    ...
                    <div className="flex flex-row items-end justify-between w-full h-full">
                        ...
                        <div className="flex flex-col items-center justify-between w-full h-full gap-1 pd-1.5xl:gap-3">
                            ...
                            <div className="h-full w-full flex flex-col items-center justify-start gap-3 pd-1.5xl:gap-6">
                                ...
                                >
                                    {stats.map((stat) => (
                                        <PlayerBackCardAttributeDelta
                                            newlyRevealedRatings={delta?.newly_revealed_ratings}
                                            key={stat.title}
                                            field={stat.field}
                                            title={stat.title}
                                            deltaStat={displayPlayerStat(stat.field, delta?.player)}
                                            value={displayPlayerStat(stat.field, player)}
                                            delta={delta ? delta[stat.deltaField] : ''}
